...
export const selectStructureDef = {
  ...
  inputSchema: {
    ...
    properties: {
      ...
      story_name: { type: 'string', description: 'Name of the story to export to' },
...
export async function executeSelectStructure(args: any) {
  const { premise, designing_principle = 'None provided', story_name = 'default_story' } = args;
  ...
  try {
    ...
    return {
      content: [{ type: 'text', text: `Structure brief successfully generated and saved to workspace for story: ${story_name}.` }]
    };
  } catch (error: any) {
    return {
      content: [{ type: 'text', text: `Error running select_structure: ${error.message}` }],
      isError: true
    };
  }
}
