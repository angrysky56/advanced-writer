...
export async function executeReviewNarrative(args: any) {
  const { text, scope, story_id = 'default_story', scene_id = 'unknown_scene' } = args;
  ...
  try {
    ...
    await workspaceExporter.saveDiagnosticReport(story_id, scene_id, diagnosticReport);

    return {
      content: [{ type: 'text', text: `Diagnostic report generated and saved for story: ${story_id}, scene: ${scene_id}.` }]
    };
  } catch (error: any) {
    return {
      content: [{ type: 'text', text: `Error running review_narrative: ${error.message}` }],
      isError: true
    };
  }
}
