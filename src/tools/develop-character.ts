...
export const developCharacterDef = {
  ...
  inputSchema: {
    ...
    properties: {
      ...
      archetype: {
        type: "string",
        description: "For create — one of 12 Jungian archetypes",
      },
      story_name: {
        type: "string",
  ...
  try {
    if (action === "create") {
      ...
      return {
        content: [
          {
            type: "text",
            text: `Character ${name} created successfully and saved to ${story_name} workspace.`,
          },
        ],
      };
    }
    ...
    return {
      content: [
        {
          type: "text",
          text: `Action ${action} is not yet fully implemented for develop_character. Only 'create' is supported.`,
        },
      ],
    };
  } catch (error: any) {
    return {
      content: [
        {
          type: "text",
          text: `Error running develop_character: ${error.message}`,
        },
      ],
      isError: true,
    };
