...
/**
 * Refiant AI client — OpenAI-compatible chat completions API.
 * Docs: https://console.refiant.ai/docs
 * Models: protea-1 (1M ctx), protea-5 (5M ctx), protea-10 (10M ctx)
 */
export class RefiantClient {
  private apiKey = ENV.REFIANT_API_KEY;
  private baseUrl = ENV.REFIANT_BASE_URL;
  ...
  async generateCompletion(
    ...
    if (!this.apiKey) {
      throw new Error("REFIANT_API_KEY is not set");
    }
      ...
      try {
        ...
        if (!response.ok) {
          ...
          const permanent =
            ...
            response.status !== 429;
          const err = new Error(
            `Refiant API error ${response.status} ${response.statusText}${body ? `: ${body.slice(0, 800)}` : ""}`,
          );
        ...
        if (!content) {
          const err = new Error(
            `Refiant returned no content: ${JSON.stringify(data).slice(0, 800)}`,
          );
        ...
        if (attempt === maxAttempts) {
          throw new Error(
            `Refiant call failed after ${maxAttempts} attempt(s): ${reason}`,
          );
        }
        console.error(
          `[refiant] attempt ${attempt}/${maxAttempts} failed (${reason}); retrying...`,
        );
        await this.backoff(attempt);
      }
    }
    // Unreachable, but satisfies the type checker.
    throw lastErr instanceof Error ? lastErr : new Error("Refiant failed");
  }
...
export const refiantClient = new RefiantClient();
