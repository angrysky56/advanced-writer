...
Cast-injection fix (the critic confirmed this independently): the seeded graph cast wasn't reaching the prose — `create_narrative`'s scene-1 draft prompt got only logline+tone, so the model invented fresh characters and the affect system tracked ghosts. Now: cast is generated FIRST, then injected (by name/role/archetype) into both the architecture brief and the scene-1 draft prompt; `continue_narrative` injects an explicit CANON CAST list from the graph and forbids inventing new primaries. This also fixes the "Elena (brief) vs Elara (prose)" name split (arch brief now references the real cast).

Plutchik depth (toward the full wheel): added opposites, the eight primary dyads (love/submission/awe/disapproval/remorse/contempt/aggressiveness/optimism), and three intensity tiers per emotion (serenity→joy→ecstasy, etc.) in `extract.ts`, with `intensityLabel`/`deriveDyads`/`detectAmbivalence` helpers. `formatAffectProfile` now renders each emotion's intensity-tier name, the active compound emotions, and any opposed-pair ambivalence — so a character reads as feeling *remorse* or *contempt*, not just a vector of primaries.

Known fabrication still to fix (flagged): the UI **Pathologies panel** (`app/api/workspace/route.ts`) is hash-derived theater — `Moralizing Ending` is literally `hash % 3 === 0`, and the cortisol/oxytocin/dopamine scores fall back to name-hash. The real evaluation is the StoryScope reports. Fixing the panel means emitting structured scores from the diagnostic prompts and parsing those (next pass).
