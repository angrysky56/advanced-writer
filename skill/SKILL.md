...
11b. **compile_revision_plan**: (Re)compile the revision plan from an EXISTING StoryScope review — no lenses re-run, fast and cheap. Use when a review finished but its plan failed to compile, or to regenerate the plan after review reports were edited. Overwrites storyscope-reports/<version>/revision-plan.json.
